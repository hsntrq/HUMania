...
{
    // it will display pigeon on (x, y) location, the size of pigeon is 70 width, 60 height
    moverRect = {x, y, 70, 60};
}

Nest::Nest()
{ //default constructor
    // it will display pigeon on x = 30, y = 40 location, the size of pigeon is 70 width, 60 height
    moverRect = {30, 40, 70, 60};
}

bool Nest::is_delete() //nest is never deleted while the game is running
{
    return false;
}